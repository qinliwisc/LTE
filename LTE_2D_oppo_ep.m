function LTE_2D_oppo_ep(ep_value,Nr_value,Nv_value,freq,delta_pos)
...
epsilon = 2^(-ep_value);
sigma = @(x,y,km)(1./(sin(km*pi*x)+1.5)+1+y-y);
% sigma = inline('x-x+1');
sigma_x = sigma(xx_s,yy_s,freq)-sigma(xx_s,yy_s,freq)+1;


boundary_left = zeros(Nr_value+1,1);% boundary_left(delta_pos) = 1;
boundary_right = zeros(Nr_value+1,1); boundary_right(delta_pos) = 1;
boundary_bottom = zeros(Nr_value+1,1);% boundary_bottom(delta_pos) = 1;
...
save(['test_2D_left/test_g_right',num2str(delta_pos),'_ep_',num2str(ep_value),'.mat']);
