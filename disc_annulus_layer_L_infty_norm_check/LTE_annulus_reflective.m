function extrapo = LTE_annulus_reflective(ep_value,N_value)
...
Lr = 1; Nr = 10*N_value; dr = 1/Nr; r0 = 0:dr:Lr; r0 = r0 + 1;
...
extrapo = f(:,Nr/2,:); extrapo = reshape(extrapo,Na,Nt); extrapo = sum(sum(extrapo))/Na/Nt;
save(['annulus/reflective_ep',num2str(ep_value),'_',num2str(N_value),'.mat']);
...
print(gcf,'-depsc2',['annulus/reflective_ep',num2str(ep_value),'_',num2str(N_value),'.eps']);
close(handle_f);
