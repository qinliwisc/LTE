function MsTransport_2D
% set up
Lx = 1;
Nx_H = 60; dx_H = 2*Lx/Nx_H; x0_H = -Lx:dx_H:Lx;
[x0,y0] = meshgrid(x0_H,x0_H); [x0s,y0s] = meshgrid(x0_H(2:end-1),x0_H(2:end-1));
Nx_h = 10; dx_h = dx_H/Nx_h; x0_h = 0:dx_h:Lx;
[x0h,y0h] = meshgrid(x0_h,x0_h);

sigma_fun = @(x,y)cos(10*pi*x).*cos(4*pi*y)+5;

% stiffness matrix construction
Sigma_mat = zeros((Nx_H+1)^2,(Nx_H+1)^2);
Sigma_x_mat = zeros((Nx_H+1)^2,(Nx_H+1)^2);
for km = 1:Nx_H % for y
    for kn = 1:Nx_H % for x
        [indices] = index_map(km,kn,Nx_H);
        ...
        Sigma_x_mat(indices,indices) = Sigma_x_mat(indices,indices) - Sigma_x_mat_temp/3;
    end
end

index_matrix = [1:(Nx_H+1)^2]; index_matrix = reshape(index_matrix,Nx_H+1,Nx_H+1); index_matrix = index_matrix';
remainder = index_matrix(2:end-1,2:end-1); remainder = remainder(:);
Sigma_mat_s = Sigma_mat(remainder,remainder); Sigma_x_mat_s = Sigma_x_mat(remainder,remainder);

% evolution
t = 0; t_Final = 1; dt = dx_H/10; time_counter = 1;
rho_ini = cos(pi*x0s/2).*cos(pi*y0s/2);

rho = cell(1,t_Final/dt); rho{1} = rho_ini; rho_old = rho_ini(:);
Sigma_inv = Sigma_mat_s - dt*Sigma_x_mat_s;
while t < t_Final
    rho_old = Sigma_mat_s*rho_old;
    rho_new = Sigma_inv\rho_old;
    mesh(x0s,y0s,reshape(rho_new,Nx_H-1,Nx_H-1));pause(0.01);
    rho{time_counter+1} = reshape(rho_new,Nx_H-1,Nx_H-1); rho_old = rho_new;
    t = t+dt; time_counter = time_counter+1;
end

save(['2D/rho_',num2str(Nx_H),'_',num2str(Nx_h),'.mat']);
