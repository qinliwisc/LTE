function MsHeat_2D_period_modify
% set up
Lx = 1; Nx_H = 50; dx_H = 2*Lx/Nx_H; x0_H = -Lx:dx_H:Lx;
[x0,y0] = meshgrid(x0_H,x0_H);
Nx_h = 10; dx_h = dx_H/Nx_h; x0_h = 0:dx_h:Lx;
[x0h,y0h] = meshgrid(x0_h,x0_h);

sigma_fun = @(x,y)cos(pi*x).*cos(pi*y)+1;
...
while t < t_Final
    ...
    figure(2); mesh(x0(1:end-1,1:end-1),y0(1:end-1,1:end-1),reshape(rho_new,Nx_H,Nx_H));title(num2str(t));pause;
    rho{time_counter+1} = reshape(rho_new,Nx_H,Nx_H); rho_old = rho_new;
    t = t+dt; time_counter = time_counter+1;
end

save(['2D/pipi/rho_',num2str(Nx_H),'_',num2str(Nx_h),'.mat']);
