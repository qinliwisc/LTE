...
epsilon_index = 0; epsilon = 10^(-epsilon_index); sigma = @(x)cos(10*pi*x)+4.1;
...
while t < tFinal
    ...
    proj_a = proj_a_new;
%     source_c = zeros(size(proj_a)); source_c(2:end,:) = -dt*proj_a(2:end,:)/epsilon;
%     source_f = Flux_A*proj_a;
%     source_f = -dt*(Sigma_x_mat_s*source_f')';
%     proj_a = epsilon*(Sigma_mat_s*proj_a')' + source_f + source_c;
%     proj_a = proj_a'/epsilon; proj_a = Sigma_mat_s\proj_a; proj_a = proj_a';
...
save(['1D/transport_',num2str(epsilon_index),'.mat']);
end
