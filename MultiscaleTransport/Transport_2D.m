...
sigma_fun = @(x,y)cos(2*pi*x).*cos(2*pi*y)+1;
epsilon_index = 2; epsilon = 10^(-epsilon_index);

t = 0; tFinal = 0.3; dt = min(dx_H/4,epsilon/2);
rho_ini = cos(pi*x0(1:end-1,1:end-1)/2).*cos(pi*y0(1:end-1,1:end-1)/2);
f_ini = zeros(Nx_H,Nx_H,Nv); proj_a = zeros(Nx_H,Nx_H,Np);
f_plane = rho_ini;
% f_v = exp(-20*v0.^2); f_v = f_v(:); rho = ones(1,Nv)*diag(W0)*f_v; f_v = 2*f_v/rho;
f_v = ones(Nv,1); rho = ones(1,Nv)*diag(W0)*f_v; f_v = 2*f_v/rho;
for km = 1:Nv
...
figure(3); mesh(proj_a(:,:,1)/sqrt(2));pause;
% flux in v
...
while t < tFinal
    ...
    proj_a = proj_a_new; rho = proj_a(:,:,1)/sqrt(2);
    figure(4)
    mesh(rho);
    pause(0.01);
end

save(['2D/transport_',num2str(epsilon_index),'.mat']);
end
