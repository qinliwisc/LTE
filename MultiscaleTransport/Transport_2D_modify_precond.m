function Transport_2D_modify_precond
...
Nx_h = 10; dx_h = dx_H/Nx_h; x0_h = 0:dx_h:Lx; [x0h,y0h] = meshgrid(x0_h,x0_h);
Nv = 10; [v0,W0] = legendre_quad(Nv-1); Np = 4; [poly_l] = legendre_poly(v0,[0:1:Np-1]);
...
epsilon_index = 1; epsilon = 10^(-epsilon_index);
...
Phi_mat = zeros((Nx_H)^2,(Nx_H)^2); Sigma_mat = zeros((Nx_H)^2,(Nx_H)^2);
...
rho_ini = 1.1+cos(pi*x0(1:end-1,1:end-1)+dx_H/2).*cos(pi*y0(1:end-1,1:end-1)+dx_H/2);
...
while t < tFinal
    ...
    data_pre = proj_a;
    data_pre(:,:,2:end) = data_pre(:,:,2:end)/(1+dt/epsilon^2);
    for km = 1:Np
        data_temp = data_pre(:,:,km); data_temp = data_temp(:);
        data_temp = Sigma_mat*data_temp;
        data_pre(:,:,km) = reshape(data_temp,Nx_H,Nx_H);
    end
    data_pre = data_pre(:);
    proj_a_new = gmres(@MAB_multi,data_pre,50,1e-6);

proj_a = reshape(proj_a_new,Nx_H,Nx_H,Np);

    rho = proj_a(:,:,1);
    figure(4); mesh(rho); pause(0.01);
end
filename = ['2D/10pi2pi/transport_',num2str(epsilon_index),'.mat'];
...
% p_temp = p; p_temp(:,:,2:end) = p_temp(:,:,2:end)/(1+dt/epsilon^2);
% term2_1 = p_temp;
% term2_2 = p_temp;
% term1 = p_temp;
for km = 1:Nx_H
...
term2_1(:,:,2:end) = term2_1(:,:,2:end)/(1+dt/epsilon^2);
term2_2(:,:,2:end) = term2_2(:,:,2:end)/(1+dt/epsilon^2);
term1 = p; term1(:,:,2:end) = term1(:,:,2:end)/(1+dt/epsilon^2);
for km = 1:Np
    data_temp = term1(:,:,km); data_temp = data_temp(:);
    data_temp = (Sigma_mat - Phi_mat)*data_temp;
    ...
    term3(:,:,km) = reshape(data_temp,Nx_H,Nx_H);
end
...
function [Sigma_mat,Sigma_x_mat,Sigma_y_mat,Phi_mat,Load] = BasisConstruct(sigma_fun,segment_x,segment_y)
    ...
    for km = 1:4
        for kn = 1:4
            ...
            pxPhiPhi = soln_c{km}.*sigma_c.*px_soln{kn};
            pyPhiPhi = soln_c{km}.*sigma_c.*py_soln{kn};
            Sigma_x_mat(km,kn) = sum(sum(pxPhiPhi))*dx_h*dx_h;
            Sigma_y_mat(km,kn) = sum(sum(pyPhiPhi))*dx_h*dx_h;
        end
    end
end
