function MsHeat_2D_period_symm
% set up
Lx = 1;
...
for km = 1:Nx_H % for y
    for kn = 1:Nx_H % for x
        ...
        [Phi_mat_temp,Sigma_x_mat_temp,~] = BasisConstruct(sigma_fun,segment_x,segment_y);
...
% evolution
t = 0; t_Final = 0.1; dt = dx_H/4; time_counter = 1;
...
Sigma_inv = Phi_mat - dt*Sigma_x_mat;
while t < t_Final
    rho_old = Phi_mat*rho_old;
    rho_new = Sigma_inv\rho_old;
    figure(1); mesh(x0(1:end-1,1:end-1),y0(1:end-1,1:end-1),reshape(rho_new,Nx_H,Nx_H));
    title(num2str(time_counter));pause(0.01);
    rho{time_counter+1} = reshape(rho_new,Nx_H,Nx_H); rho_old = rho_new;
    t = t+dt; time_counter = time_counter+1
end

save(['2D/ex3_heat_symm/rho_',num2str(Nx_H),'_',num2str(Nx_h),'.mat']);

end

function [Phi_mat,Sigma_x_mat,Load] = BasisConstruct(sigma_fun,segment_x,segment_y)
    ...
    % storing basis
    soln = cell(1,4); Phi_mat = zeros(4,4); Sigma_x_mat = zeros(4,4);
...
%         mesh(xx,yy,soln_temp);pause;
    ...
    for km = 1:4
        for kn = 1:4
            ...
            Phi_mat(km,kn) = sum(sum(PhiSigmaPhi))*dx_h*dx_h;
