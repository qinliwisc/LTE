...
% Heat_compatible_initial;
% for k=6:8
%     sigma_value = 2^k;
%     epsilon_value = 2^(-k);
%     Reference_compatible_initial(sigma_value,epsilon_value);
% end

threshold = 0.95;

% filename = ['data/heat_compatible_initial/heat_1n3'];
% heat = load(filename);
% x = heat.x; theta_heat = heat.theta; theta_heat = theta_heat(:);
% theta_heat_t = theta_heat((x>-threshold)&(x<threshold));


filename = ['data/heat_compatible_initial/v=1ini=0_t_3n3/heat_1n4'];
heat = load(filename);
x = heat.x; theta_heat = heat.theta; theta_heat = theta_heat(:);
...
filename = ['data/heat_compatible_initial/v=1ini=0_t_3n3/heat_5n4'];
heat = load(filename);
x = heat.x; theta_heat = heat.theta; theta_heat = theta_heat(:);
...
for k = 6:8
    km = k-5;
    sigma_value = 2^k;
    filename = ['data/heat_compatible_initial/v=1ini=0_t_3n3/transport_',num2str(sigma_value)];
...
xlabel('x');
ylabel('\Theta');
print(gcf,'-depsc2', 'data/heat_compatible_initial/v=1ini=0_t_3n3/pure_heat.eps');
close(h);
...
xlim([-1 -0.9]);
ylabel('\Theta');
print(gcf,'-depsc2', 'data/heat_compatible_initial/v=1ini=0_t_3n3/pure_heat_zoom.eps');
close(h);
...
print(gcf,'-depsc2','data/heat_compatible_initial/v=1ini=0_t_3n3/error.eps');
close(h);