...
fboundary_l.fun =  inline('100*abs(v)*t+1.5','v','t');
fboundary_r.fun =  inline('100*abs(v)*t+1.5','v','t');
boundary_l = feval(fboundary_l.fun,Vp,0);
boundary_r = feval(fboundary_r.fun,Vm,0);

fboundary_fund.fun =  inline('abs(v)','v','t');
...
while t<Time
    ...
    theta(1) =  eta_fund*100*t+1.5;
    theta(end) = eta_fund*100*t+1.5;

%     plot(x, theta,'.-.');pause(0.005);
    
    t = t+dt
end
save('data/heat_compatible/heat_1n3');
return