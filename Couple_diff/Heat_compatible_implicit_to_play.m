function Heat_compatible_implicit_to_play(cases,epsilon,fboundary_l,fboundary_r,phi_x,phi_v,ChangeInTime)
%%=============
% heat equation, kinetic boundary
%%=============

% discretization x, odd number of grid points
dx = 5e-3; x = -1+dx/2:dx:1-dx/2; Nx = length(x);
theta = x-x;
...
while t<Time
    ...
    load = theta;
    load(1) = load(1) + 4*dt/5/dx/(1.2*extrapolation)/epsilon*eta_l;
    load(end) = load(end) + 4*dt/5/dx/(1.2*extrapolation)/epsilon*eta_r;
...
save(['data/heat_compatible_to_play/case',num2str(cases),'/heat_implicit_5n3_',num2str(1/epsilon)]);
return