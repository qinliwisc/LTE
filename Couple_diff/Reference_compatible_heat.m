function Reference_compatible_heat(sigma_value,epsilon_value)
% %=============
% reference solution
% linear transport
...
% boundary data
fboundary_l.fun =  inline('abs(v)','v');
fboundary_r.fun =  inline('abs(v)','v');
boundary_l = feval(fboundary_l.fun,Vp);
boundary_r = feval(fboundary_r.fun,Vm);

[boundary_fund_adjust,eta] = boundary_computing(V_full,Weight,H_poly,boundary_l);


% initial data
phi = ones(Np,Nx);
phi = abs(V_full)*ones(1,Nx)*eta+eta/2;
