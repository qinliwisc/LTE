...
%     sigma_value = 2^k;
%     epsilon_value = 2^(-k);
%     Reference_compatible(sigma_value,epsilon_value);
% end

threshold = 0.9;

filename = ['data/heat_compatible/heat_1n3'];
heat = load(filename);
...
for k = 6:8
    km = k-5;
    sigma_value = 2^k;
    filename = ['data/heat_compatible/transport_',num2str(sigma_value)];
...
xlabel('x');
ylabel('\Theta');
print(gcf,'-depsc2', 'data/heat_compatible/pure_heat.eps');
close(h);
...
xlim([-1 -0.9]);
ylabel('\Theta');
print(gcf,'-depsc2', 'data/heat_compatible/pure_heat_zoom.eps');
close(h);
...
print(gcf,'-depsc2','data/heat_compatible/error.eps');
close(h);