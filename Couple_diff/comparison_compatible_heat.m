...
% 
% Heat_compatible_heat;
% for k=6:8
%     sigma_value = 2^k;
%     epsilon_value = 2^(-k);
%     Reference_compatible_heat(sigma_value,epsilon_value);
% end

threshold = 0.97;

filename = ['data/heat_compatible_heat/heat_1n3'];
heat = load(filename);
...
for k = 6:8
    km = k-5;
    sigma_value = 2^k;
    filename = ['data/heat_compatible_heat/transport_',num2str(sigma_value)];
...
xlabel('x');
ylabel('\Theta');
print(gcf,'-depsc2', 'data/heat_compatible_heat/pure_heat.eps');
close(h);
...
xlim([-1 -0.9]);
ylabel('\Theta');
print(gcf,'-depsc2', 'data/heat_compatible_heat/pure_heat_zoom_l.eps');
close(h);
...
xlim([0.9 1]);
ylabel('\Theta');
print(gcf,'-depsc2', 'data/heat_compatible_heat/pure_heat_zoom_r.eps');
close(h);
...
print(gcf,'-depsc2','data/heat_compatible_heat/error.eps');
close(h);